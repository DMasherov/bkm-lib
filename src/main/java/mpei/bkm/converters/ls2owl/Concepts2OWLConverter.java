...
public class Concepts2OWLConverter implements Converter<LSOntology, Set<OWLAxiom>> {
    private OWLOntologyManager manager;
    private OWLOntology owlOntology;
    private OWLDataFactory df;

    public Concepts2OWLConverter(OWLOntologyManager manager, OWLOntology owlOntology) {
    ...
    public Set<OWLAxiom> convert(LSOntology lsOnt) throws UnconvertableException {
        ...
        for (Named classTerm : lsOnt.getClassesTerms()) {
            declareAxioms.addAll(createDeclareAxioms(classTerm.getBkmClass()));
        }
        for (mpei.bkm.model.lls1.terms.l.Named linkTerm : lsOnt.getBinaryLinksTerms()) {
